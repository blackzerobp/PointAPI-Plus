...
class PointAPI extends PluginBase implements Listener{
	...
	public function createAccount($player, $defaultPoint = false, bool $force = false) : bool{
		...
		if(!$this->provider->accountExists($player)){
			...
			$this->getServer()->getPluginManager()->callEvent($ev = new CreateAccountEvent($this, $player, $defaultPoint, "none"));
	...
	public function setPoint($player, $amount, bool $force = false, string $issuer = "none") : int{
		...
		if($this->provider->accountExists($player)){
			...
			$this->getServer()->getPluginManager()->callEvent($ev = new SetPointEvent($this, $player, $amount, $issuer));
			if(!$ev->isCancelled() or $force === true){
				$this->provider->setPoint($player, $amount);
				$this->getServer()->getPluginManager()->callEvent(new PointChangedEvent($this, $player, $amount, $issuer));
	...
	public function addPoint($player, $amount, bool $force = false, $issuer = "none") : int{
		...
		if(($point = $this->provider->getPoint($player)) !== false){
			...
			$this->getServer()->getPluginManager()->callEvent($ev = new AddPointEvent($this, $player, $amount, $issuer));
			if(!$ev->isCancelled() or $force === true){
				$this->provider->addPoint($player, $amount);
				$this->getServer()->getPluginManager()->callEvent(new PointChangedEvent($this, $player, $amount + $point, $issuer));
				return self::RET_SUCCESS;
			}
			return self::RET_CANCELLED;
		}
	...
	public function reducePoint($player, $amount, bool $force = false, $issuer = "none") : int{
		...
		if(($point = $this->provider->getPoint($player)) !== false){
			...
			$this->getServer()->getPluginManager()->callEvent($ev = new ReducePointEvent($this, $player, $amount, $issuer));
			if(!$ev->isCancelled() or $force === true){
				$this->provider->reducePoint($player, $amount);
				$this->getServer()->getPluginManager()->callEvent(new PointChangedEvent($this, $player, $point - $amount, $issuer));
				return self::RET_SUCCESS;
			}
			return self::RET_CANCELLED;
		}
	...
	public function onEnable(){
		/*
		 * ë””í´íŠ¸ ì„¤ì • íŒŒì¼ì„ ë¨¼ì € ìƒì„±í•˜ê²Œ ë˜ë©´ ë°ì´í„° í´ë” íŒŒì¼ì´ ìžë™ ìƒì„±ë˜ë¯€ë¡œ
		 * 'Failed to open stream: No such file or directory' ê²½ê³  ë©”ì‹œì§€ë¥¼ ì—†ì•¨ ìˆ˜ ìžˆìŠµë‹ˆë‹¤
		 * - @64FF00
		 *
		 * [ì¶”ê°€ ì˜µì…˜]
	...
	private function registerCommands(){
		...
		$commands = [
			...
			"pay" => "\\onebone\\pointapi\\command\\PayCommand",
			"setlang" => "\\onebone\\pointapi\\command\\SetLangCommand",
			"mystatus" => "\\onebone\\pointapi\\command\\MyStatusCommand"
		];
